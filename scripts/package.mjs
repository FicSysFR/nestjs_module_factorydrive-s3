...
const PACKAGE_NAME = '@ficsysfr/nestjs_module_factorydrive-s3'
const CORE_NAME = '@ficsysfr/nestjs_module_factorydrive'
const REPOSITORY_URL = 'https://github.com/FicSysFR/nestjs_module_factorydrive-s3.git'
const HOMEPAGE_URL = 'https://ficsysfr.github.io/nestjs_module_factorydrive/guide/drivers'
const BUGS_URL = 'https://github.com/FicSysFR/nestjs_module_factorydrive-s3/issues'
...
export async function packageAndAudit(projectRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..')) {
  ...
  try {
    ...
    const importScript = `import('${PACKAGE_NAME}').then((module) => { if (typeof module.AwsS3Storage !== 'function') throw new Error('ESM export missing') })`
    run(process.execPath, ['--input-type=module', '--eval', importScript], temporaryRoot)
    const requireScript = `const module = require('${PACKAGE_NAME}'); if (typeof module.AwsS3Storage !== 'function') throw new Error('CommonJS export missing')`
    run(process.execPath, ['--eval', requireScript], temporaryRoot)

    await writeFile(
      join(temporaryRoot, 'types-smoke.ts'),
      `import { AwsS3Storage } from '${PACKAGE_NAME}'\nimport type { AbstractStorage } from '${CORE_NAME}'\ndeclare const concrete: AwsS3Storage\nconst base: AbstractStorage = concrete\nvoid base\n`,
    )
    await writeFile(
